...
public class RiskResourceIT {
    ...
    public void getAllRisksWithEagerRelationshipsIsEnabled() throws Exception {
        RiskResource riskResource = new RiskResource(riskRepositoryMock);
        when(riskRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restRiskMockMvc = MockMvcBuilders.standaloneSetup(riskResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restRiskMockMvc.perform(get("/api/risks?eagerload=true"))
        .andExpect(status().isOk());

        verify(riskRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllRisksWithEagerRelationshipsIsNotEnabled() throws Exception {
        RiskResource riskResource = new RiskResource(riskRepositoryMock);
            when(riskRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restRiskMockMvc = MockMvcBuilders.standaloneSetup(riskResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
