...
@RestController
@RequestMapping("/api")
public class LikeProposedProjectRisksResource {

    private final Logger log = LoggerFactory.getLogger(LikeProposedProjectRisksResource.class);
    ...
    public LikeProposedProjectRisksResource(UserService userService, ProjectRepository projectRepository, ProjectRiskService projectRiskService) {
     ...
     */
    @PostMapping("/like-project-risks")
    public ResponseEntity<ProjectRisks> addLike(@Valid @RequestBody ProjectRisks proposedProjectRisk) {
        log.debug("REST request to like proposed ProjectRisk: {}", proposedProjectRisk);

        Optional<User> user = userService.getUserWithAuthorities();
        if (!user.isPresent()) throw new BadRequestAlertException("Missing credentials", ENTITY_NAME, "usernull");

        Optional<Project> project = projectRepository.findByUsersIsContainingAndIdEquals(user.get(), proposedProjectRisk.getProject().getId());
        if (!project.isPresent()) {
            throw new BadRequestAlertException("User not in project", ENTITY_NAME, "projectnotexist");
        }

        ProjectRisks result = projectRiskService.addLikeToProposedProjectRisk(proposedProjectRisk, user.get());
